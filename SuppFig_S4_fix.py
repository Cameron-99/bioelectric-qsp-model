...
ax2.text(-0.1, 1.02, 'B', transform=ax2.transAxes, fontsize=12, fontweight='bold', va='bottom')

# === A SUBTITLE (inside axes, low) ===
ax1.text(0.5, 0.82, 'Fitness\nMSE: 0.12→0.01', transform=ax1.transAxes, fontsize=9,
         ha='center', va='center',
         bbox=dict(boxstyle='round,pad=0.15', facecolor='wheat', alpha=0.7))

# === B HEADER: use set_title so it sits ABOVE the image in the title area ===
ax2.set_title('Initial | Evolved | Target', fontsize=9, pad=6,
              bbox=dict(boxstyle='round,pad=0.25', facecolor='wheat', alpha=0.7))

# === FIGURE TITLE ===
fig.suptitle('Suppl. Fig. S4: DEAP Convergence', fontsize=11, fontweight='bold', y=0.98)

plt.subplots_adjust(left=0.08, bottom=0.15, top=0.86, right=0.92, wspace=0.35)

# === SAVE ===
plt.savefig('SuppFig_S5_timecourse.png', dpi=250, bbox_inches='tight', pad_inches=0.1)
plt.close()
